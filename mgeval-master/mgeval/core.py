...
class metrics(object):
    ...
    def bar_used_pitch(self, feature, track_num=0, num_bar=8):
        """
        ...
        for i in range(0, len(pattern[track_num])):
            if type(pattern[track_num][i]) == midi.events.TimeSignatureEvent:
                ...
                if 'time_sig' not in locals():  # set default bar length as 4 beat
                    ...
                    if int(pattern[track_num][i].tick / bar_length)>=num_bar:
                        print('Event exceeded sample length:')
                        print(pattern[track_num][i])
                        continue
    ...
    def bar_used_note(self, feature, track_num=0, num_bar=8):
        """
    ...
    def bar_pitch_class_histogram(self, feature, track_num=0, bpm=120, num_bar=8):
        """
