...
def stream_to_tstep1(notes_to_parse,resolution):
...
def stream_to_tstep2(notes_to_parse,resolution):
    dur = int((notes_to_parse[-1].offset+notes_to_parse[-1].quarterLength)*resolution)
    notes_list=np.ones(dur,dtype='int16')*128
    prev=128
...
def stream_to_event1(notes_to_parse,resolution):
    ...
    for n in notes_to_parse:
      ...
      if isinstance(n,chord.Chord):
          if n.offset>prev:
              notes_list.append(note_length_event(n.offset-prev,resolution))
          if n.offset>=prev:
              notes_list.append(int(n[-1].pitch.midi))
              notes_list.append(note_length_event(n.quarterLength,resolution))
              notes_list.append(int(n[-1].pitch.midi)+128)
              prev=n.offset+n.quarterLength
      return notes_list
  
def get_notes_from(encoding,resolution,data_path,save=True,streams=False):
    notes=[]
    bad_files=[]
    count=0
    if streams:
    ...
    for file in data:
        try:
            ...
            if encoding=='tstep1':
                notes_list=stream_to_tstep1(notes_to_parse, resolution)
            elif encoding=='tstep2':
                notes_list=stream_to_tstep2(notes_to_parse, resolution)
            elif encoding=='event1':
                notes_list=stream_to_event1(notes_to_parse, resolution)
            else:
                print('Bad encoding')
                return
    ...
    if save:
      savepath=os.path.join('notes','notes_'+encoding+'_res'+str(resolution))
...
def get_notes(encoding,data_dir='midi/midi/session/dummy',file_extension='.mid',resolution=8,streams=False):
