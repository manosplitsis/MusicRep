...
def make_abc_seeds(text_path,no_seeds,seq_length):
    ...
    #pieces_validate=pieces_validate_c+pieces_validate_csharp
    del pieces
    #inds=np.random.randint(0,len(pieces_validate),size=no_seeds)
    inds=pieces_validate[:no_seeds]
    seeds=[]
    for i in inds:
        seeds.append(pieces_validate_c[i].split()[start_index: start_index + seq_length])
    return seeds

def make_abc_seeds_bars(text_path,no_seeds,bar_length):
    print('making seeds of bar length: ',bar_length)
    text=load_doc(text_path)
    chars = sorted(list(set(text)))
    #start_index = random.randint(0, len(text) - seq_length - 1)
    start_index=0
    ...
    print('choosing from ',len(pieces_validate),'pieces')
    #pieces_train_csharp=pieces_csharp[0:len(pieces_csharp)-int(val_split*len(pieces_csharp))]
    #pieces_validate_csharp=pieces_csharp[len(pieces_csharp)-int(val_split*len(pieces_csharp)):len(pieces_csharp)]
    #pieces_train=pieces_train_c+pieces_train_csharp
    #pieces_validate=pieces_validate_c+pieces_validate_csharp
    del pieces
    #inds=np.random.randint(0,len(pieces_validate),size=no_seeds)
    inds=np.arange(no_seeds)
    seeds=[]
    for i in inds:
        seed_piece=pieces_validate[i]
        seed=cut_after_bars(seed_piece, bar_length)
        seeds.append(seed)
    return seeds
#%%
def cut_after_bars(abc,bars):
    abc=abc.split()
    count_bars=0
    if not abc[0]=='<s>':
        print('no start symbol')
    if '|' in abc[1]:
        bars+=1
...
def generate_bars(model_path,seeds,temp=1.,generate_pieces=True,output_bars=8):
    ...
    for i in range(len(sentences)):
        count_bars=seed_length
        while count_bars<=output_bars:
            sentence=sentences[i]
            x_pred=np.reshape(sentences[i],(1,len(sentences[i])))
            preds=model.predict(x_pred)
            next_index=sample(preds[0][-1],temp)
            next_char=indices_char[next_index]
            if next_char=='</s>' or next_char=='<s>':
                continue
            if '|' in next_char:
                count_bars+=1
            sentences[i]=np.append(sentences[i],next_index)
            seeds[i].append(next_char)
        abc+='\n\n'+process_abc(seeds[i],count_abc)
        count_abc+=1
    print(abc)
    with open(experiment_path+'/output_seedlenbars_'+str(seed_length)+'.abc','w') as f:
            f.write(abc)
    print('saved at: '+experiment_path)
...
if __name__=='__main__':
    model_path='models/data_V3_nohead_model_n1_s32_d0.2_sl100_bs256_C_run_0/models/model-164-1.7136-1.6604'
    text_path='data/text_44_nohead'
    no_seeds=200
    seq_length=1
    bar_length=1
    #seeds=make_abc_seeds(text_path, no_seeds, seq_length)
    #tt=generate_many_abc(model_path, seeds, seq_length)
    seeds=make_abc_seeds_bars(text_path, no_seeds, bar_length)
    generate_bars(model_path,seeds,output_bars=8)