...
from tensorflow.keras.layers import Input, Dense, Dropout, LSTM, Activation, Bidirectional, Flatten, AdditiveAttention,TimeDistributed, Embedding
...
def build_model(batch_size, seq_len, n_vocab,lstm_size=256,lstm_no=1,dropout_rate=0.2):
    ...
    if lstm_no==1:
        model.add(LSTM(lstm_size,return_sequences=False,input_shape=(None,n_vocab)))
...
def build_model2(batch_size, seq_len, n_vocab,lstm_size=256,lstm_no=1,dropout_rate=0.2):
    '''
    Build the model--> Embedding the every character as neuron Dense Layer
                      --> 3 Layer of LSTM each of 256 units(cell) in it.
                      --> Each in between layer of LSTM Dropout with probabilty of 0.2
    ...
    model = Sequential()
    #model.add(Embedding(n_vocab, 256))
